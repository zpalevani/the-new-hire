...
This is the module where people who "don't code" build software — by managing, not typing. You'll direct your agent to construct a real, running triage workflow, and you'll make the single most important design decision in applied AI along the way.
...
The agent will propose a plan involving terminal commands. **This is the moment the workshop's promise gets kept:** the terminal exists, the commands run — but your agent operates them while you supervise from the chat, reviewing each step in request-review mode. Read the plan, approve, watch.
...
```
Run linting on the project and fix anything it flags. Summarize what
you checked in two sentences.
```
...
```
Launch the local development playground so I can chat with the triage
agent in my browser. Give me the link to click.
```
